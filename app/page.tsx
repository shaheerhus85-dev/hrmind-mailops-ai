...
function InboxView({items,selected,reviewed,query,onQuery,onSelect,onAnalyze,onDraft,onReviewed,onKeep}:{items:EmailItem[];selected:EmailItem;reviewed:number[];query:string;onQuery:(s:string)=>void;onSelect:(e:EmailItem)=>void;onAnalyze:()=>void;onDraft:()=>void;onReviewed:()=>void;onKeep:()=>void}){
  ...
  <section className="list-panel panel"><div className="list-toolbar"><div><small>Queue</small><strong>Hiring inbox</strong><em>{items.length} classified threads</em></div><div className="queue-tools"><label><Search/><input value={query} placeholder="Filter" onChange={e=>onQuery(e.target.value)}/></label><button type="button" aria-label="Sort newest first">Newest <ChevronDown/></button></div></div><div className="scroll-list">{items.map(item=><button key={item.id} className={clsx("email-row",selected.id===item.id&&"active")} onClick={()=>onSelect(item)}><span className="queue-avatar"><Avatar name={item.sender}/><i className={item.priority.toLowerCase()}/></span><span><b>{item.sender}<small>{item.time}</small></b><strong>{item.subject}</strong><em>{item.preview}</em><span className="row-tags"><Tag>{item.category}</Tag>{reviewed.includes(item.id)&&<Tag>Reviewed</Tag>}</span></span></button>)}{items.length===0&&<EmptyResults label="No recruiter emails match these filters."/>}</div></section>
  <section className="detail-panel panel"><div className="detail-scroll"><div className="detail-heading"><div><Status tone={selected.priority==="High"?"amber":"blue"}>{selected.priority} priority</Status><h2>{selected.subject}</h2><p>From {selected.sender} · To Recruitment Team · Today at {selected.time}</p></div><strong>{selected.confidence}%</strong></div><div className="email-tags"><Tag>{selected.category}</Tag><Tag>Confidence {selected.confidence}%</Tag><Tag>{selected.attachment==="No attachment"?"No CV detected":"CV detected"}</Tag><Tag>{selected.attachment==="No attachment"?"No attachment":"1 attachment"}</Tag></div><div className="email-body"><p>Hello Recruitment Team,</p><p>{selected.preview.replace(/\.{3}$/,".")}</p><p>Please let me know if you need further information before the next step.</p></div><div className="reason-card"><Sparkles/><div><strong>AI classification reason</strong><p>{selected.reason}</p></div><Status tone="blue">{selected.confidence}%</Status></div></div><div className="action-bar">{selected.category==="CV Application"&&<Button onClick={onAnalyze}>Analyze candidate</Button>}<Button secondary={selected.category==="CV Application"} onClick={onDraft}>Generate reply draft</Button><Button quiet onClick={onReviewed}><CheckCircle2/> Mark reviewed</Button><button className="text-action" onClick={onKeep}>Keep in queue</button></div></section>
</div>}
...
function CandidateView({items,selected,onSelect,onInterview,onDraft,onReviewed}:{items:Candidate[];selected:Candidate;onSelect:(c:Candidate)=>void;onInterview:()=>void;onDraft:()=>void;onReviewed:()=>void}){
  ...
  return <div className="candidate-view"><section className="candidate-table panel"><div className="table-head"><span>Candidate</span><span>Role</span><span>Workflow</span><span>Score</span><span>Recommendation</span><span>Next step</span></div><div className="scroll-list">{items.map(c=><button className={clsx("candidate-row",selected.id===c.id&&"active")} key={c.id} onClick={()=>onSelect(c)}><span className="person"><Avatar name={c.name}/><strong>{c.name}</strong></span><span>{c.role}</span><span>{c.source}</span><b>{c.score}%</b><Status tone={c.score>=90?"green":"amber"}>{c.reviewStatus==="reviewed"?"reviewed":c.reviewStatus==="analyzed"?"analyzed":c.status}</Status><span className="next-step">{c.reviewStatus==="reviewed"?"Complete":c.score>=90?"Interview kit":"Recruiter review"}</span></button>)}</div></section><section className="candidate-focus panel"><div className="candidate-focus-scroll"><div className="focus-top"><Avatar name={selected.name} large/><div><h2>{selected.name}</h2><p>{selected.role} · {selected.location} · {selected.experience}</p></div><div className="candidate-score"><small>Match score</small><strong>{selected.score}%</strong></div></div><p className="summary">{selected.summary}</p><SkillBlock title="Matched skills" items={selected.skills}/><SkillBlock title="Missing skills" items={selected.gaps} warning/><div className="candidate-risk"><AlertTriangle/><div><strong>Risk note</strong><p>{selected.risk}</p></div></div><div className="candidate-disclaimer"><ShieldCheck/><div><strong>Human review disclaimer</strong><p>This is decision support only. A recruiter must verify the profile before any hiring action.</p></div></div></div><div className="focus-actions"><Button onClick={onInterview}>Open interview kit</Button><Button secondary onClick={onDraft}>Generate reply draft</Button><Button quiet onClick={onReviewed}>Mark reviewed</Button></div></section></div>
}
...
function Status({children,tone}:{children:ReactNode;tone:"green"|"amber"|"blue"}){return <span className={clsx("status",tone)}>{children}</span>}
